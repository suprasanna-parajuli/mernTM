...
const Layout = ({ onLogout, user }) => {
  ...
  const { isStudying } = useTimer();
  const location = useLocation();

  // Pages that should NOT show the right sidebar
  const hideRightSidebarPaths = ["/schedule", "/subjects", "/materials", "/analytics", "/ai-insights"];
  ...
  // STATISTICS CARD - Fixed: Added return and capitalized component name
    ...
    return (
      <div className="p-2 sm:p-3 rounded-xl bg-white shadow-sm border border-purple-100 hover:shadow-md transition-all duration-300 hover:border-purple-200 group">
        <div className="flex items-center gap-2">
          <div className="p-1.5 rounded-lg bg-gradient-to-br from-fuchsia-500/10 to-purple-500/10 group-hover:from-fuchsia-500/20 group-hover:to-purple-500/20 transition-colors">
            {icon}
          </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="ml-0 xl:ml-64 lg:ml-64 md:ml-16 pt-16 p-3 sm:p-4 md:p-3 transition-all duration-300">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 sm:gap-6">
            ...
            <div className="bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-purple-100">
              <h3 className="text-base sm:text-lg font-semibold mb-3 sm:mb-4 text-gray-800 flex items-center gap-2">
                <TrendingUp className="size-4 sm:size-5 text-purple-500" />
                Task Statistics
              </h3>
              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6">
                <StatCard
                  ...
                  icon={
                    <Circle className="size-3.5 sm:size-4 text-purple-500" />
                  }
                />
                <StatCard
                  title="Completed"
                  ...
                  icon={
                    <Circle className="size-3.5 sm:size-4 text-green-500" />
                  }
                />
                <StatCard
                  title="Pending"
                  ...
                  icon={
                    <Circle className="size-3.5 sm:size-4 text-fuchsia-500" />
                  }
                />
                <StatCard
                  title="Rate"
                  value={`${stats.completionPercentage}%`}
                  icon={<Zap className="size-3.5 sm:size-4 text-purple-500" />}
                />
              </div>
              ...
              <div className="space-y-2 sm:space-y-3">
                <div className="flex items-center justify-between text-gray-700">
                  <span className="text-xs sm:text-sm font-medium flex items-center gap-1.5">
                    <Circle className="size-2.5 sm:size-3 text-purple-500 fill-purple-500" />
                    Task Progress
                  </span>
              ...
              <div className="space-y-2 sm:space-y-3">
                {tasks.slice(0, 3).map((task) => (
                  <div
                    ...
                    className="flex items-center justify-between p-2 sm:p-3 hover:bg-purple-50/50 rounded-lg transition-colors duration-300 border border-transparent hover:border-purple-100"
                  >
                ...
                {tasks.length === 0 && (
                  <div className="text-center py-4 sm:py-6 px-2">
                    <div className="size-12 sm:size-16 mx-auto mb-4 rounded-full bg-purple-100 flex items-center justify-center">
                      <Clock className="size-6 sm:size-8 text-purple-500" />
                    </div>
      ...
      {/* Floating Timer Widget - Visible globally when studying */}
      {isStudying && <FloatingTimer onRefresh={fetchTasks} />}
    </div>
  );
};
